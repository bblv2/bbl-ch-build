...
# to atl_settings.py for ch-atl* hostnames. Hostnames not matching the
# dispatchers prefixes (e.g. ch-test-*) need overrides delivered via the
# always-loaded bbl/local_settings.py path (settings.py imports it twice
# unconditionally, last at line ~793, after all dispatcher branches). To
# inherit the prod-flavored atl_settings while still overriding DB +
# secrets, we load it explicitly at the top of local_settings.py.
HOST_SETTINGS="$DJANGO_DIR/bbl/local_settings.py"
cat > "$HOST_SETTINGS" <<PYSETTINGS
"""Auto-generated by bbl-ch-build step 05 at $(date -u +%Y-%m-%dT%H:%M:%SZ).

Loaded by bbl/settings.py via 'from local_settings import *' (twice,
unconditionally). Inherits prod-flavored atl_settings then layers the
per-host DATABASES + secrets from /etc/bbl-ch-host.conf."""
from atl_settings import *
