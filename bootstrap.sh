...
# Provisioning args come from /etc/bbl-build-ch-bootstrap.env that the
# linode-cli wrapper writes via user_data. If absent, fall back to
# defaults so manual SSH-and-rerun works too.
ENV_FILE=/etc/bbl-build-ch-bootstrap.env
if [[ -r "$ENV_FILE" ]]; then
    # shellcheck disable=SC1090
    . "$ENV_FILE"
fi

: "${BBL_BUILD_REPO:=https://github.com/bblv2/bbl-build-ch.git}"
...
mkdir -p /usr/src
git clone --branch "$BBL_BUILD_BRANCH" "$BBL_BUILD_REPO" /usr/src/bbl-build-ch

exec /usr/src/bbl-build-ch/setup.sh \
    role="$BBL_ROLE" \
    size="$BBL_SIZE" \
    hostname="$BBL_HOSTNAME" \
    2>&1 | tee /var/log/bbl-build-ch.log
